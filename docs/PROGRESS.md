...
## 오늘 한 일 / 내일 할 일 (2026-08-05 기준, 사용자 요청 정리)
**오늘 한 일 (모두 패치 전달, 대부분 병합됨):**
- 재현성 번들(9) → CSV 데이터 핀(10) → KRX 휴장일 수정(11) → 입력 UX: 팝업 캘린더+전략 프리셋(12)
- KRX 대장정: 캘린더 클릭 버그(라벨 가로채기) 수정, KRX 스냅샷 엔드포인트 전멸 진단 →
  **명시적 종목 바스켓**으로 우회, 거래대금/빈프레임 정규화, **원주가→수정주가 폴백** → **KRX 백테스트 완주**.
- 스크리너 수정(8): 실패 노출 + 빠른 조건 프리셋 + KRX 바스켓 + 친절 에러. **(마지막 패치 `screen_fixes.patch` 전달, 병합 대기)**

**내일 할 일 (우선순위):**
1. 사용자가 `screen_fixes.patch` 병합 후, **/screen에서 "빠른 조건" 골라 CSV/KRX로 종목찾기 동작 확인**. 안 되면 화면에 뜨는 실패 메시지 받아서 이어서 디버그.
2. 스크리너도 **재현성 번들 kind="screen"** 이미 기록 중 — KRX/CSV 바스켓 실행의 데이터 핀은 다음 후보.
3. 그다음 제품 슬라이스 선택지(사용자가 고르게): **페이퍼 트레이딩** / **결과 내보내기(CSV·JSON)** / **스크린·KRX 데이터 핀** / **리서치 파이프라인(리밸런스별 PIT 유니버스)**.
4. 만약 스크리너 새 에러가 나오면, `run_screen`의 데이터 소스별 경로(특히 KRX 바스켓 + 종목별 시세)부터 확인.

**세션이 끊겼다 복귀하면:** 이 문서 위→아래로 읽고, "요청 백로그"에서 미완([ ]) 항목부터. 패치는 항상
`git fetch` 후 최신 base에서 생성(과거 중복-커밋 충돌 교훈). read-only 토큰이라 push 대신 패치 전달.
